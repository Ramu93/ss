...
$thead = $tbody = '';$datatableflag = false;$data = $engineer_stock = $store_material_stock = array();
...
//Fetch Store Stock
$stock_query = "SELECT * FROM store_material_stock";
$stock_result = mysqli_query($dbc, $stock_query);
if(mysqli_num_rows($stock_result)>0){
    while ($stock_row = mysqli_fetch_assoc($stock_result)) {
        $store_material_stock[$stock_row['item_master_id']] = $stock_row;
    }
}
...
$thead = '<tr><th>Engineer</th><th>Material Name</th><th>Requested Quantity</th><th>Issued Quantity</th><th>Available Quantity</th><th>Action</th>';
$snocount = 1;
if(mysqli_num_rows($result)>0){
    ...
    foreach ($data as $key => $value) {
        ...
        foreach ($value as $key2 => $value2) {
            ...
            if(array_key_exists($value2['item_master_id'], $store_material_stock)){
                $available_stock = $store_material_stock[$value2['item_master_id']]['current_stock'];
            }else{
                $available_stock = 0;
            }
            if(array_key_exists($value2['engineer_id'], $engineer_stock) && array_key_exists($value2['item_master_id'], $engineer_stock[$value2['engineer_id']])){
                ...
                if($needed_qty > 0){
                    if($tc1 == 1){
                        $tbody .= '<tr><td rowspan="[rs_engineer]">'.$value2['employee_name'].'</td><td>'.$value2['item_name'].'</td><td>'.$needed_qty.'</td>
                        <td><input type="text" class="number" name="issued_qty_'.$ukey.'" /></td><td>'.$available_stock.'</td><td><input type="checkbox" name="issue_list[]" value="'.$ukey.'" />
                        ...
                        $tbody .= '<tr><td>'.$value2['item_name'].'</td><td>'.$needed_qty.'</td><td><input type="text" class="number" name="issued_qty_'.$ukey.'" /></td><td>'.$available_stock.'</td><td><input type="checkbox" name="issue_list[]" value="'.$ukey.'" />
                        <input type="hidden" name="engineer_id_'.$ukey.'" value="'.$value2['engineer_id'].'" /><input type="hidden" name="item_master_id_'.$ukey.'" value="'.$value2['item_master_id'].'" /></td></tr>';
                    }
                    $tc1++;
                }
                ...
                if($tc1 == 1){
                    ...
                    $tbody .= '<tr><td>'.$value2['item_name'].'</td><td>'.($value2['quantity']-$value2['replaced_quantity']).'</td><td><input type="text" class="number" name="issued_qty_'.$ukey.'" /></td><td>'.$available_stock.'</td><td><input type="checkbox" name="issue_list[]" value="'.$ukey.'" />
                    <input type="hidden" name="engineer_id_'.$ukey.'" value="'.$value2['engineer_id'].'" /><input type="hidden" name="item_master_id_'.$ukey.'" value="'.$value2['item_master_id'].'" /></td></tr>';
                }
                $tc1++;
            }
    ...
    if($tbody == '') $tbody = '<tr><td colspan="5">Pending Materials have been already issued</td></tr>';
}else{
    $tbody = '<tr><td colspan="5">No Pending Material Request</td></tr>';
}
