        ...
        foreach ($data as $key => $value) {
            ...
            foreach ($value as $key2 => $value2) {
                if(array_key_exists($value2['engineer_id'], $engineer_stock) && array_key_exists($value2['item_master_id'], $engineer_stock[$value2['engineer_id']])){
                    ...
                    if($tc1 == 1){
                        $tbody .= '<tr><td rowspan="[rs_engineer]">'.$value2['employee_name'].'</td><td>'.$value2['item_name'].'</td><td>'.($value2['quantity']-$value2['replaced_quantity']).'</td></tr>';
                    }
                    else{
        ...
        while($row = mysqli_fetch_assoc($result)){
            if(array_key_exists($row['item_master_id'], $item_data)){
                ...
                if(($row['quantity']-$row['replaced_quantity'])>0)
                    $tbody .= '<tr><td>'.$row['item_name'].'</td><td>'.($row['quantity']-$row['replaced_quantity']).'</td></tr>';
            }
        }
    }else{
