...
if($list_type == 'engineer'){
    ...
    $query = "SELECT a.item_master_id , b.item_name , a.ticket_id, a.quantity,d.customer_name, c.engineer_id, a.replaced_quantity, a.returned_quantity, a.spare_request_id
    ...
    if(mysqli_num_rows($result)>0){
        ...
        while($row = mysqli_fetch_assoc($result)){
            /*if(array_key_exists($row['ticket_id'], $data) && array_key_exists($row['item_master_id'], $data[$row['ticket_id']])){
                ...
                $data[$row['ticket_id']][$row['spare_request_id']] = $row;
            //}
        ...
        foreach ($data as $key => $value) {
            ...
            foreach ($value as $key2 => $value2) {
                if($tc1 == 1){
                    $tbody .= '<tr><td rowspan="[rs_ticket]">'.$ticket_id.'</td><td rowspan="[rs_customer]">'.$value2['customer_name'].'</td><td>'.$value2['item_name'].'</td><td>'.($value2['replaced_quantity']-$value2['returned_quantity']).'</td>
                    <td><input type="text" class="number" name="returned_qty_'.$key2.'" /></td><td><input type="checkbox" name="return_list[]" value="'.$key2.'" />
                        <input type="hidden" name="spare_request_id_'.$key2.'" value="'.$key2.'" /></td></tr>';
                }
                else{
                    $tbody .= '<tr><td>'.$value2['item_name'].'</td><td>'.($value2['replaced_quantity']-$value2['returned_quantity']).'</td><td><input type="text" class="number" name="returned_qty_'.$key2.'" /></td><td><input type="checkbox" name="return_list[]" value="'.$key2.'" />
                        <input type="hidden" name="spare_request_id_'.$key2.'" value="'.$key2.'" /></td></tr>';
                }
                $tc1++;
            }
    ...
    $query = "SELECT a.item_master_id , b.item_name , SUM(a.replaced_quantity) as replaced_quantity, SUM(a.returned_quantity) as returned_quantity
            ...
            WHERE a.item_master_id = b.item_master_id AND b.is_returnable = 'yes' AND (a.replaced_quantity - a.returned_quantity) > 0
...
<!--End-breadcrumbs-->
    ...
    <form id="return_material_form" action="" onsubmit="return false;" method="POST">
        ...
        <?php $display = ($list_type == 'ticket')?'display:block;':'display:none;'; ?>
        <div class="row-fluid" style="padding:25px;<?php echo $display; ?>" >
            <div class="span8"></div>
            <div class="span3 text-right">
                <button type="button" class="btn btn-success" onclick="return_material();" id="btn_return_material">Return Material</button>
                <input type="hidden" name="action" value="return_material" />
                <label for="return_list[]" generated="true" class="error"></label>
            </div>
        </div>
        </form>
	</div>
...
$(document).ready(function() {
    ...
    $('#return_material_form').validate({
        ...
        messages: { 
                "return_list[]": "Please select at least one material."
        } 
    });
});
...
function return_material(){
    if($('#return_material_form').valid()){
        data = $('#return_material_form').serialize();
        $.ajax({
            ...
            success: function(result){
                if(result.infocode == "RETURNMATERIALSUCCESS"){
